...
backtrace_hashes = (
    set()
)  # Set for backtrace's hashes. If current bactrace's hash is not in set then add this backtrace to bactraces list, otherwise this backtrace will be passed
...
def compile_driver_worker(bgen_args):
    # Always set introspector environment variable
    constructed_env = {**os.environ, "FUZZ_INTROSPECTOR": "1"}
...
parser.add_argument(
    "-i", "--include", default="include", help="paths for including when compiling"
)

parser.add_argument("-a", "--static_path", help="path to folder of installed libraries")

parser.add_argument(
    "-asan", "--futag_asan", action="store_true", help="Compile with futag ASAN"
)

parser.add_argument(
    "-s", "--syslibs", default="", help="list of system libs for compiling"
)

parser.add_argument(
    "-so", "--objects", default="", help="list of object files for compiling"
)

parser.add_argument(
    "-gdb", "--gdb_debug", action="store_true", help="Option for debugging with gdb"
)
...
parser.add_argument(
    "-f", "--fork", type=int, default=0, help="Fork option for libFuzzer"
)

parser.add_argument(
    "-to", "--timeout", type=int, default=10, help="Time out for fuzzing"
)
parser.add_argument(
    "-tt",
    "--max_total_time",
...
parser.add_argument(
    "-o", "--output", default="futag-targets", help="Folder for generating fuzz-targets"
)

parser.add_argument(
    "-m", "--memlimit", type=int, default=2048, help="Memory limit for fuzzing"
)
...
parser.add_argument(
    "-c", "--coverage", action="store_true", help="Option for counting coverage"
)
...
parser.add_argument(
    ...
    type=lambda p: Path(p).absolute(),
    default=False,
    help="The path to fuzz introspector bins directory (collect info about generate fuzzing drivers)",
)

parser.add_argument(
    "header", type=lambda p: Path(p).absolute(), help="Header file for fuzzing"
)
...
if not (
    (args.package / "bin/clang").exists()
    and (args.package / "bin/futag-gen4libfuzzer").exists()
    and (args.package / "bin/futag-gen4crusher").exists()
):
    parser.error(
...
if (args.target != "libFuzzer") and (args.target != "Crusher"):
    parser.error("Unknown what type of targets (%s) for generating!" % args.target)
    exit()
...
if args.static_path:
    ...
    for p in paths:
        for alib in [
            x.as_posix() for x in Path(p).absolute().glob("**/*.a") if x.is_file()
        ]:
...
compiler = (args.introspect if args.introspect else args.package) / "bin/clang"
...
if not args.nocompiling:
    ...
    # To support fuzz-introspector - build in LTO mode
    if args.introspect:
        makefile_compiler_flags += " -flto"
    ...
    if static_libs:
        ...
        if args.futag_asan:
            makefile_static_list += (
                args.package / "lib/clang/11.1.0/lib/linux/libclang_rt.asan-x86_64.a"
            ).as_posix()
            ...
            if static_libs:
                ...
                if args.futag_asan:
                    compile_command.append(
                        (
                            args.package
                            / "lib/clang/11.1.0/lib/linux/libclang_rt.asan-x86_64.a"
                        ).as_posix()
                    )
    ...
    with Pool(multi) as p:
        p.map(compile_driver_worker, compile_list)

    # Extract the results of compilation
    compile_targets_list = [x for x in targets_folder.glob("**/*.out") if x.is_file()]
    print(
        "done! (compiled "
...
if args.introspect:
        call(
            [
                "python3",
                "/home/Futag/fuzz-introspector/post-processing/main.py",
                "correlate",
                "--futag_mode=True",
                f"--binaries_dir={str(targets_folder)}"
            ]
        )
