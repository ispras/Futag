...
class Futag4CrusherVisitor : public RecursiveASTVisitor<Futag4CrusherVisitor> {
 private:
  ASTContext *astContext;  // used for getting additional AST info

 public:
  // ostream create_fuzz_target(string file_path);
  explicit Futag4CrusherVisitor(CompilerInstance *CI)
...
class Futag4CrusherASTConsumer : public ASTConsumer {
 private:
  Futag4CrusherVisitor *visitor;  // doesn't have to be private

 public:
  explicit Futag4CrusherASTConsumer(CompilerInstance *CI)
      : visitor(new Futag4CrusherVisitor(CI)) {}
  virtual void HandleTranslationUnit(ASTContext &Context) {
    visitor->TraverseDecl(Context.getTranslationUnitDecl());
  }
};

class Futag4CrusherFrontendAction : public ASTFrontendAction {
 public:
  virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(
      clang::CompilerInstance &Compiler, llvm::StringRef InFile) {
    return std::unique_ptr<clang::ASTConsumer>(
        new Futag4CrusherASTConsumer(&Compiler));
  }
};

static cl::OptionCategory FutagCategory("futag-gen4crusher options");
...
static cl::extrahelp MoreHelp(
    "\n Futag-gen4crusher is used to generate Crusher targets for all APIs "
...
int main(int argc, const char **argv) {
  ...
  Tool.run(newFrontendActionFactory<Futag4CrusherFrontendAction>().get());
  ...
  for (vector<futag::FunctionDetail>::reverse_iterator f_iter =
    ...
    for (vector<futag::TypeSequence>::iterator p_iter = f_iter->params.begin();
      ...
      for (vector<futag::QualTypeDetail>::reverse_iterator qt_iter =
        ...
        if (qt_iter->generator_type == futag::GEN_STRING) {
          gen_string_4Crusher(cur_var_name, *qt_iter, generator);
          last_var_name = cur_var_name;
        }
    ...
    futag::gen_wrapper_4Crusher(fuzz_file, include_headers, generator);
    fuzz_file->close();
    delete generator;
    delete fuzz_file;
  }
