...
tc = ToolchainConfig.from_futag_llvm("Futag/futag-llvm/") #Путь к рабочей директории futag
g = Generator(
"path/to/library/source/code", #Путь к директории исходных текстов исследуемого приложения
...
generator = Generator(library_root)
...
generator = FuzzDataProviderGenerator(library_root)
...
ctx_gen = ContextGenerator(library_root)
ctx_gen.gen_context()
