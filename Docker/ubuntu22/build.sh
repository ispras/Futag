...
cmake  -G "Unix Makefiles" -DLLVM_BUILD_TESTS=OFF -DLLVM_TARGETS_TO_BUILD=X86 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$futag_install_folder -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DLLVM_INCLUDE_BENCHMARKS=OFF -DLLVM_EXTERNAL_PROJECTS="clang;compiler-rt;" -DLLVM_EXTERNAL_CLANG_SOURCE_DIR=$custom_llvm/clang  -DLLVM_EXTERNAL_COMPILER_RT_SOURCE_DIR=$custom_llvm/compiler-rt $custom_llvm/llvm
