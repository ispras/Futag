# Оглавление
For English please follow this link: [README.en](README.en.md)
- [Оглавление](#оглавление)
- [1. Описание](#1-описание)
- [2. Установка](#2-установка)
  - [2.1. Использование докер-контейнера](#21-использование-докер-контейнера)
  - [2.2. Использование предварительно упакованного пакета](#22-использование-предварительно-упакованного-пакета)
  - [2.3. Сборка и установка из исходного кода](#23-сборка-и-установка-из-исходного-кода)
    - [2.3.1. Зависимости](#231-зависимости)
    - [2.3.2. Сборка и установка](#232-сборка-и-установка)
- [3. Примеры использования](#3-примеры-использования)
  ...
  - [3.2. Автоматическая генерация фаззинг-оберток в наличии потребительской программы](#32-автоматическая-генерация-фаззинг-оберток-в-наличии-потребительской-программы)
- [4. Авторы](#4-авторы)
...
graph TD
    ...
    subgraph "Уровень 1: Инфраструктура сборки"
        D["build-llvm / build.sh — Загрузка и патч LLVM 14/18/19"]
    end
...
Вы можете попробовать собрать Futag с готовыми [Докер-файлами](https://github.com/ispras/Futag/tree/main/integration-tests/build-test) для ОС Ubuntu.

## 2.2. Использование предварительно упакованного пакета
- Загрузите последнюю версию [futag-llvm.3.0.1.tar.xz](https://github.com/ispras/Futag/releases/tag/v3.0.1) и разархивируйте. В результате инструмент будет установлен в директорию futag-llvm.
...
```
- Подготовьте директорию "build-llvm", запустив скрипт:
```bash
  ~/Futag/build-llvm$ ./prepare.sh
```
Этот скрипт создает директорию Futag/build и копирует  в неё скрипт Futag/build-llvm/build.sh
...
- Практикум по использованию FUTAG доступен в репозитории: [workshop/](./workshop/)
...
Документация Python-пакета находится [по ссылке](https://github.com/ispras/Futag/tree/main/futag-package)

Подобную информацию о работе Futag можно прочитать [по ссылке](https://github.com/ispras/Futag/blob/main/docs/how-to-work-with-futag.md)

Шаблон скриптов запуска можно посмотреть [здесь](https://github.com/ispras/Futag/blob/main/scripts/template-script.py)

Был создан [репозиторий](https://github.com/thientc/Futag-tests) для тестирования Futag над библиотеками (json-c, php, FreeImage, и т.д.), можете протестировать с [Докер-контейнером](https://github.com/ispras/Futag/tree/main/integration-tests/libraries-test).

## Документация

- [Архитектура проекта](docs/architecture.md) — трёхуровневая архитектура, поток данных
- [С/C++ анализаторы](docs/checkers.md) — FutagAnalyzer и FutagConsumerAnalyzer
- [Система генераторов](docs/generators.md) — паттерн BaseGenerator и бэкенды
- [Справочник Python API](docs/python-api.md) — полное описание классов и методов
- [Руководство для разработчиков](CONTRIBUTING.md) — настройка среды и стиль кода
...
Если вы используете FUTAG в своих исследованиях или при сообщении об ошибках, найденных в FUTAG, пожалуйста, ссылайтесь на следующие работы:
...
Благодарим за признание работы авторов при использовании FUTAG или сообщении об ошибках, обнаруженных с помощью этого инструмента.

- Доклад на Открытой конференции ИСП РАН им. В.П. Иванникова 2022
